    ...
    <header className="sticky top-0 z-50 bg-white dark:bg-[#0a0a0f] border-b border-slate-200 dark:border-white/[0.08]">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <Link href="/" className="flex-shrink-0 flex items-center" aria-label="FoodSwap home">
          <FoodSwapLogo height={22} className="text-slate-900 dark:text-white sm:h-[26px]" />
        </Link>

        {/* Segmented nav */}
        <nav className="flex items-center border border-slate-200 dark:border-white/[0.1] rounded-lg overflow-hidden bg-white dark:bg-white/[0.03] flex-shrink-0">
          {navLinks.map(({ href, label, icon: Icon }, idx) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  "relative flex items-center gap-1.5 px-2.5 sm:px-3.5 py-2 text-sm font-medium transition-colors duration-100",
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          {!loading && isAdmin && (
            <Link
              href="/admin"
              className={cn(
                "flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-md text-sm font-semibold transition-colors border",
          ...
          {!loading && (
            user ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                {/* Customer "My Orders" — medium screens+ */}
                {!isAdmin && (
                  <Link
                    href="/orders"
                    className={cn(
                      "hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold transition-colors border",
                      pathname === "/orders"
                ...
                {/* Avatar chip — customers only */}
                {!isAdmin && (
                  <Link
                    href="/account"
                    className={cn(
                      "flex items-center gap-1.5 sm:gap-2 pl-1.5 sm:pl-2 pr-2 sm:pr-3 py-1.5 rounded-md border transition-colors",
                      pathname === "/account"
                  ...
                  >
                    ...
                    <p className={cn(
                      "hidden sm:block text-[12px] font-semibold truncate max-w-[80px] md:max-w-[120px]",
                      pathname === "/account"
              ...
              <Link
                href="/login"
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-md text-sm font-semibold bg-brand-500 text-white hover:bg-brand-600 transition-colors shadow-sm"
              >
                <LogIn size={13} />
